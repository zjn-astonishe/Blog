---
title: Linux内核的I2C子系统详解 - 代码实例详解(以RK3568为例)
...
RK平台的I2C总线控制器并非直接集成在CPU内部，而是挂载到platform总线上，所以可以将RK3568 Linux的I2C驱动划分为两个部分实现：
* 采用platform驱动框架设计总线驱动，以挂载I2C适配器到platform总线。
* 采用I2C驱动框架设计设备驱动，以挂载设备到I2C总线。
...
## 5.5. i2c_adapter添加函数i2c_add_adapter()
...
### 5.5.2. 已指定ID号i2c_adapter注册函数__i2c_add_numbered_adapter()
...
### 5.5.3. i2c_adapter注册函数i2c_register_adapter()
...
### 5.5.4. 解析从设备，构建i2c_client函数of_i2c_register_devices()
